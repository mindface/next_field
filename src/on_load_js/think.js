...
export function initParticle(canvasElement) {
  ...
  function calc() {
    ...
    initParticle = Math.floor((canvas.width / 300 * ratio) + (canvas.height / 300 * ratio));
  ...
  function draw() {
    ...
    for (let index = 0; index < initParticle; index++) {
      ...
      if(_p.position.x > canvas.width + _p.circle) aryParticle[index].position.x = canvas.width - (_p.circle * 2);
      if(_p.position.y < -_p.circle) aryParticle[index].position.y = _p.circle;
      if(_p.position.y > canvas.height + _p.circle) aryParticle[index].position.y = canvas.height - (_p.circle * 2)
